...
# Usage: ./gitlab-reset.sh https://gitlab.com/user/project
...
if echo "$PROJECT_DATA" | grep -q '"message"'; then
    print_warning "Project does not exist"

    PROJECT_NAME=$(basename "$PROJECT_PATH")

    print_status "Creating public project: $PROJECT_NAME"

    CREATE_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
        --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
        --header "Content-Type: application/json" \
        --data "{\"name\":\"$PROJECT_NAME\",\"path\":\"$PROJECT_NAME\",\"visibility\":\"public\"}" \
        "$GITLAB_API/projects")
...
PROJECT_DESC=$(echo "$PROJECT_DATA" | grep -o '"description":"[^"]*"' | head -1 | cut -d'"' -f4)
...
PAYLOAD="{\"name\":\"$PROJECT_NAME\",\"path\":\"$PROJECT_NAME\",\"visibility\":\"public\",\"description\":\"$PROJECT_DESC\"}"
...
echo ""
print_success "Project recreated successfully!"
echo ""
